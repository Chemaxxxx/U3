...
public class LoteriaSolucion {	
	...
	public static void main(String[] args) {
		...
		String[] listaNumerosSorteo = new String[MAX_PREMIOS + 6]; // [i][0]: número agraciado  [i][1]: premio
		String[] listaPremiosSorteo = new String[MAX_PREMIOS + 6];
		...
		try {
			...
			for (int i=0;i<MAX_PREMIOS;i++) {
				...
				if(premio.equals("PRIMER PREMIO" )||premio.equals("SEGUNDO PREMIO")||premio.equals("TERCER PREMIO")) {
					if(numeroAgraciado>0) {
						String anterior = "" + (numeroAgraciado-1);
						while(anterior.length()< 5) {
							anterior = "0" + anterior;
						}
						
						String premioAprox = "";
						if (premio.equals("PRIMER PREMIO"))premioAprox = "Aproximación primer premio(2000 €)";
						if (premio.equals("SEGUNDO PREMIO"))premioAprox = "Aproximación segundo premio(1250 €)";
						if (premio.equals("TERCER PREMIO"))premioAprox = "Aproximación tercer premio(960 €)";
						
						
						listaNumerosSorteo[j] = anterior;
						listaPremiosSorteo [j] = "Premio anterior al gordo";
						j++;
						System.out.println("Numero : " + anterior + " numero agraciado anterior con: "  + premioAprox);
						
					}
				
				
				if(numeroAgraciado < MAX_NUMEROS - 1) {
					String posterior = "" + (numeroAgraciado+1);
					while(posterior.length()< 5) {
						 posterior = "0" + posterior;
					}
					
					int centenaPremiada = numeroAgraciado/100;
					String premioCentena = "";
					if(premio.equals("PRIMER PREMIO"))
						premioCentena = "Aproximación centena primer premio: (100€)";
					
					if(premio.equals("SEGUNDO PREMIO"))
						premioCentena = "Aproximación centena segundo premio(100€)";
					
					if(premio.equals("TERCER PREMIO"))
						premioCentena = "Aproximación centena tercer premio(100€)";
					
					for(int num = 0; num < MAX_PREMIOS; num++) {
						int centena = num/100;
						if(centena == centenaPremiada && num != numeroAgraciado) {
					...
					listaNumerosSorteo[j] = posterior; 
					listaPremiosSorteo [j] = "Premio posterior al gordo";
					j++;
					System.out.println("Numero : " + posterior + " numero agraciado posterior con: " + premioAprox);
			...
			String premioAprox = "";
			if(premio.equals("pedrea"))
				premioAprox = "Aproximación pedrea(100€)";
